...
export function computeKPIs(sessions, events) {
  ...
  const currentlyInside = sessions.filter(
    (s) => s.status === "CURRENTLY_INSIDE" || s.status === "STALE_INSIDE"
  ).length;

  const parkedDurations = sessions
    .filter((s) => s.status === "PARKED" && s.durationMinutes)
    .map((s) => s.durationMinutes);
  const avgParkedDuration =
    parkedDurations.length > 0
...
export function computeTrendData(sessions, granularity) {
  const grouped = {};
...
export function computeFlowDistribution(sessions) {
  const flows = {
...
export function computeGateLoad(sessions) {
...
export function detectAnomalies(sessions, events, unlinkedParking, confidenceThreshold = 0.75) {
  const alerts = [];
  let alertId = 0;

  // Low confidence
  events
    .filter((e) => (e.confidence || 1) < confidenceThreshold)
    .forEach((e) => {
      alertId++;
      alerts.push({
        id: `alert-${alertId}`,
  ...
  // Stale inside
  sessions
    .filter((s) => s.status === "STALE_INSIDE")
    .forEach((s) => {
      alertId++;
      alerts.push({
        id: `alert-${alertId}`,
  ...
  Object.values(completedByPlate).forEach((plateSessions) => {
    plateSessions.sort((a, b) => new Date(a.exitTime).getTime() - new Date(b.exitTime).getTime());
...
export function computePlateInsights(sessions, plate) {
...
export function computeVehicleTypeSegmentation(sessions) {
  const byType = {};
...
export function computeRepeatVisitors(sessions, minSessions = 3) {
  const byPlate = {};
  sessions.forEach((s) => {
...
export function computeRankings(sessions) {
  const parkedFreq = {};
  const passedFreq = {};
  ...
  const topParked = Object.entries(parkedFreq)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 10)
    .map(([plate, count]) => ({ plate, count }));
  const topPassed = Object.entries(passedFreq)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 10)
    .map(([plate, count]) => ({ plate, count }));
