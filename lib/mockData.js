...
function generatePlateProfile(plate) {
  return {
    plate,
...
const plateProfiles = {};
PLATES.forEach((p) => {
  plateProfiles[p] = generatePlateProfile(p);
});

function makeEvent(timestamp, plate, location, direction, confidence) {
  ...
  return {
    ...
    confidence:
      confidence ??
      (Math.random() > 0.1 ? 0.8 + Math.random() * 0.2 : 0.5 + Math.random() * 0.25),
    imageUrl: undefined,
  };
}
...
export function generateHistoricalEvents(days = 30) {
  ...
  for (let i = 0; i < 15; i++) {
    ...
    events.push(
      makeEvent(t, unknownPlate, randomFrom(PARKING_LOCATIONS), "INTERNAL", 0.5 + Math.random() * 0.3)
    );
  }
