...
export function useInsights(sessions, events, unlinkedParking, granularity) {
  ...
  const vehicleTypeByDirection = useMemo(() => computeVehicleTypeByDirection(events), [events]);
  ...
  return { kpis, gateTrends, parkingTrends, gateLoad, flowDist, alerts, vehicleTypes, vehicleTypeByDirection, repeatVisitors, rankings, gatePeakHours };
}
