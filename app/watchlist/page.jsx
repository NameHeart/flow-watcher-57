        ...
        {watchlistData.length === 0 ? (
          <div className="rounded-xl border bg-card p-8 sm:p-12 text-center shadow-card">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Add plates from the Investigation Drawer or Insights page.
            </p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                    ...
                    <span className="font-mono font-bold text-base sm:text-lg truncate">
                      {d.plate}
                    </span>
                    <span className="text-xs text-muted-foreground hidden sm:inline">
                      {d.vehicleType} · {d.color}
                    </span>
                  </div>
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <button
                      onClick={() => setInspectPlate(d.plate)}
                      className="p-2 rounded-lg hover:bg-muted transition-colors"
                    >
                      <Eye className="h-4 w-4 text-muted-foreground" />
                    </button>
                    <button
                      onClick={() => handleRemove(d.plate)}
