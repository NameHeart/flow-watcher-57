...
function InsightsContent() {
  ...
  const { alerts, vehicleTypes, repeatVisitors, rankings } = useInsights(
    sessions,
    events,
    unlinkedParking,
    "daily"
  ...
  const vehicleTypeNames = useMemo(
    () => [...new Set(sessions.map((s) => s.vehicleType))],
    [sessions]
  );
      ...
      <div className="space-y-4">
        ...
        <Tabs defaultValue="search">
          <TabsList className="bg-muted w-full sm:w-auto overflow-x-auto">
            <TabsTrigger value="search" className="gap-1 sm:gap-1.5 text-xs sm:text-sm">
              <Search className="h-3 w-3 sm:h-3.5 sm:w-3.5" />
              <span className="hidden xs:inline">Search</span>
            </TabsTrigger>
            <TabsTrigger value="segments" className="gap-1 sm:gap-1.5 text-xs sm:text-sm">
              <BarChart3 className="h-3 w-3 sm:h-3.5 sm:w-3.5" />
              <span className="hidden xs:inline">Segments</span>
            </TabsTrigger>
            <TabsTrigger value="repeat" className="gap-1 sm:gap-1.5 text-xs sm:text-sm">
              <Users className="h-3 w-3 sm:h-3.5 sm:w-3.5" />
              <span className="hidden xs:inline">Repeat</span>
            </TabsTrigger>
            <TabsTrigger value="rankings" className="gap-1 sm:gap-1.5 text-xs sm:text-sm">
          ...
          <TabsContent value="search" className="space-y-4 mt-4">
            <div className="relative w-full sm:max-w-md">
              ...
              <Input
                placeholder="Search plate (e.g. ABC)..."
                value={searchQuery}
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        <span className="text-xs text-muted-foreground ml-2 hidden sm:inline">
                          {r.vehicleType} · {r.color}
                        </span>
                      </div>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          addToWatchlist(r.plate);
                        }}
          ...
          <TabsContent value="segments" className="space-y-4 mt-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="rounded-xl border bg-card p-3 sm:p-4 shadow-card min-w-0">
                <h3 className="font-display text-xs sm:text-sm font-semibold mb-3">
                  Parked vs Passed by Type
                </h3>
              ...
              <table className="w-full text-sm min-w-[400px]">
                <thead>
                  <tr className="bg-muted/50 border-b">
                    <th className="text-left px-3 sm:px-4 py-2.5 font-medium text-muted-foreground text-xs sm:text-sm">
                      Type
                    </th>
                    <th className="text-left px-3 sm:px-4 py-2.5 font-medium text-muted-foreground text-xs sm:text-sm">
                      Total
                ...
                <tbody>
                  {vehicleTypes.map((v) => (
                    <tr key={v.type} className="border-b hover:bg-muted/30">
                      ...
                      <td className="px-3 sm:px-4 py-2.5 text-muted-foreground text-xs sm:text-sm">
                        {v.total}
                      </td>
                      <td className="px-3 sm:px-4 py-2.5 text-success text-xs sm:text-sm">
                        {v.parked}
                      </td>
                      <td className="px-3 sm:px-4 py-2.5 text-info text-xs sm:text-sm">{v.passed}</td>
                      <td className="px-3 sm:px-4 py-2.5 text-muted-foreground text-xs sm:text-sm">
                        {v.avgDuration ? `${v.avgDuration} min` : "—"}
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              {repeatVisitors.length === 0 ? (
                ...
                <div className="space-y-1">
                  {repeatVisitors.map((r) => (
                    ...
                    >
                      <span className="font-mono text-xs sm:text-sm font-semibold truncate min-w-0">
                        {r.plate}
                      </span>
          ...
          <TabsContent value="rankings" className="space-y-4 mt-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <RankingCard
                title="Top Parked (by frequency)"
                data={rankings.topParked}
                onInspect={setInspectPlate}
              />
              <RankingCard
                title="Top Passed Through"
                data={rankings.topPassed}
                onInspect={setInspectPlate}
              />
            </div>
          </TabsContent>
    ...
    <div className="rounded-xl border bg-card shadow-card">
      ...
      <div className="divide-y">
        {data.map((r, i) => (
          <div
            key={r.plate}
            onClick={() => onInspect(r.plate)}
            className="flex items-center gap-3 px-3 sm:px-4 py-2.5 hover:bg-muted/30 cursor-pointer"
          >
            <span className="w-5 text-xs font-bold text-muted-foreground">#{i + 1}</span>
            <span className="font-mono text-xs sm:text-sm font-semibold flex-1 truncate min-w-0">
              {r.plate}
            </span>
        ...
        {data.length === 0 && (
          <p className="p-4 text-sm text-muted-foreground text-center">No data</p>
        )}
      </div>
    </div>
  );
